...
use crate::commands::command_ctx::CommandCtx_GetRedisCtx;
...
impl QueryCtx {
    ...
    pub fn set_global_execution_context(
        ...
        self.query_data.query = command_ctx.query;
        self.global_exec_ctx.bc = command_ctx.bc as *mut _;
        self.global_exec_ctx.redis_ctx = CommandCtx_GetRedisCtx(command_ctx) as *mut _;
        self.global_exec_ctx.bolt_client = command_ctx.bolt_client as *mut _;
        self.global_exec_ctx.command_name = command_ctx.command_name as *mut _;
    ...
    #[inline]
    fn get_undo_log(&mut self) -> query_ctx_struct::UndoLog {
    ...
    fn update_stage_duration(&mut self) {
        unsafe {
            let ms = simple_toc(self.stats.timer.as_mut_ptr()) * 1000.0;
